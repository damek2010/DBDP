...
class Zadanie_edit_c extends CI_Controller {
	...
	public function edycja()
	{
		if(!$this->session->userdata('log_zal'))
		{
			...
			{
				$pytanie = 'UPDATE Zadania SET procent_wykoanania = "'. $_POST["textinput0"] .'", czas_trwania = STR_TO_DATE("'. $_POST["textinput1"] .'","%Y-%m-%d"), kupka = "'. $_POST["kupka"] .'", Stany_id_stanu = "'.$_POST["textinput5"].'", Projekty_id_projektu= "'.$_POST["textinput6"].'" WHERE id_zadania = "' . $_POST["selectbasic"] . '"';
				echo $pytanie;
